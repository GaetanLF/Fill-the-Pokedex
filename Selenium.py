...
for Pkmn in df['Name'].tolist(): # For each Pokemon's name in the list
    ...
    try:
        for sVar in range(0,len(scrappedVariables),1): # For each variable in the scrapped list
            scrappedVariables[sVar] = driver.find_element_by_xpath(xpath=xPathList[sVar]).text # We scrap.
...
#-------------------- New Xpaths for Mega Pokemons ---------------------------

xPathList = ['/html/body/div[1]/div[2]/div/div[7]/div/div[1]/div[4]/table[2]/tbody/tr[6]/td[1]/table/tbody/tr[3]/td[2]',
...
meansScrapped = df.groupby(['Generation']).agg({'height':'mean','weight':'mean','catchRate':'mean',
                                'maleRatio':'mean','hatchSteps':'mean'})
